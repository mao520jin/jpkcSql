...
<script type="text/javascript" src="<%=basePath %>/js/front/resources.js"></script>
...
<body onload="loadData(1,10,'<%=basePath %>/frontResources/resourcesList', ${type });"> 
	...
	<div id="content" class="col-xs-10" style="width: 85%;">  
		<div class="row-fluid">
			<h3 style="text-align: center;">
				<c:if test="${type eq '1' }">电子教案</c:if>
				<c:if test="${type eq '2' }">教学课件</c:if>
				<c:if test="${type eq '3' }">教学视频</c:if>
				<c:if test="${type eq '4' }">教学大纲</c:if>
				<c:if test="${type eq '5' }">实验教学资料</c:if>
			</h3>  
			
			<div class="col-xs-12"><div class="info survey">
				<div class="details" id="resources_content"></div>
...
<!-- 表单提交预览图片，避免超链接显示path -->
<form id="view_office_form" name="view_office_form"  method="post"  action="" style="display: none">
