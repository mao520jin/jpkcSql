		...
		if(data.length > 3) {
			for(var i=0; i<3; i++) {
				...
				var content = {
					...
					"id": data[i].id,
					"basePath": "http://localhost:8080/jpkc/frontNotice/noticeDetail/" + data[i].id
				};
				notice.push(content); 
			}
		}else{
...
<script id="list" type="text/x-handlebars-template">
	{{#each notice}}
		<dl>
			<dd>
				<a href="{{basePath}}">{{title}}</a><span>{{time}}</span>
			</dd>
		</dl>
	{{/each}}
</script>
			...
			<div class="col-xs-5">
				<div class="right">
					<div class="info notice">
						<h3>通知公告<a href="<%=basePath%>/frontNotice/noticesIndex">More...</a></h3>
